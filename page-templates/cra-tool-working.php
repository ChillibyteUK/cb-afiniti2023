<?php
/**
 * Template Name: CRA Tool (working)
 *
 * Work in progress. The phase 1 hardening of the CRA tool template, parked here
 * while page-templates/cra-tool.php stays on the earlier version.
 *
 * Differs from the live template by:
 *  - posting to admin-post.php (action=cb_cra_submit) rather than at cra.php
 *  - sending the reCAPTCHA token so the server can verify it
 *  - showing ?cra_error= messages instead of silently bouncing to the home page
 *  - reading the reCAPTCHA site key from Site-Wide Settings > CRA Tool
 *  - using PerformanceNavigationTiming rather than the deprecated
 *    performance.navigation.type
 *
 * Named distinctly so it does not collide with the live template in the page
 * template dropdown. See CLAUDE.md.
 *
...
<main id="main">
    ...
    <?php
    /*
     * A rejected submission comes back here with ?cra_error=. Previously any
     * failure silently redirected to the home page, so the visitor lost five
     * steps of input with no idea why.
     */
    $cra_errors = array(
...
?>
                ...
                <div class="form_buttons text-end">
                    <?php
                    /*
                     * Posts to admin-post.php rather than straight at the theme
                     * file, so WordPress is loaded and routing applies. cra.php
                     * still works for anyone served cached HTML with the old
                     * action - see inc/cb-cra-submit.php.
                     */
                    ?>
...
<script>
    window.addEventListener('pageshow', function(event) {
        ...
        var wasBackForward = event.persisted || (nav && nav.type === 'back_forward');
