...
<main id="main">
	...
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- read-only display of an error flag; the submission endpoint does its own token validation.
	...
	<div class="container-xl">
		<div class="alert alert-danger d-block my-4" role="alert">
			<?php echo esc_html( $cra_errors[ $cra_error ] ); ?>
		</div>
	</div>
		<?php
	}
	...
	<section id="hero" class="hero d-flex align-items-start pt-lg-0 align-items-lg-center">
		<div class="hero__inner container-xl text-center">
			<h1 class="mb-3"><?php echo wp_kses_post( get_the_title() ); ?></h1>
			<div class="hero__cta">
	...
	<section class="stepCard" id="form0">
		<?php echo apply_filters( 'the_content', serialize_blocks( $cra_intro_blocks ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
	</section>
	<div class="container-xl">
		<?php
		/*
		...
		$cra_step_titles = array( '' !== $cra_org_title ? $cra_org_title : 'About Your Organisation' );

		foreach ( $cra_question_steps as $cra_qstep ) {
			$cra_step_titles[] = $cra_qstep['title'];
		}

		$cra_step_titles[] = '' !== $cra_contact_title ? $cra_contact_title : 'Your Details';
		...
		$cra_open_step = function ( $index, $kind ) use ( $cra_step_titles ) {
			...
			?>
		<section class="stepCard" id="craStep<?php echo (int) $index; ?>" data-cra-step="<?php echo (int) $index; ?>"
			data-cra-kind="<?php echo esc_attr( $kind ); ?>">
			<h2>Step <?php echo (int) $index + 1; ?> - <?php echo esc_html( $title ); ?></h2>
			...
			<ol class="cra_steps" aria-label="Assessment progress">
				...
				foreach ( $cra_step_titles as $i => $step_title ) {
					...
					?>
				<li class="cra_steps__step <?php echo esc_attr( $state ); ?>"
					<?php echo 'is-current' === $state ? ' aria-current="step"' : ''; ?>>
					<span class="cra_steps__marker" aria-hidden="true"><?php echo (int) $i + 1; ?></span>
					<span class="cra_steps__label">
						<?php echo esc_html( $step_title ); ?>
						<?php if ( $note ) { ?>
						<span class="visually-hidden">(<?php echo esc_html( $note ); ?>)</span>
						<?php } ?>
					</span>
				</li>
					<?php
		...
		$cra_step_buttons = function ( $index, $next = 'Next' ) {
			...
			<div class="form_buttons d-flex gap-2 justify-content-between">
				<?php if ( 0 === $index ) { ?>
				<a href="<?php echo esc_url( get_permalink() ); ?>" class="btn btn-secondary">Back</a>
				<?php } else { ?>
				<button type="button" class="btn btn-secondary" data-cra-back>Back</button>
				<?php } ?>
				<button type="button" class="btn btn-primary" data-cra-next><?php echo esc_html( $next ); ?></button>
			</div>
			<?php
		};
		...
		foreach ( $cra_question_steps as $cra_qstep ) {
			...
			<div class="form_panel">
				...
				<div class="alert alert-light">
					<?php echo wp_kses_post( $cra_qstep['header'] ); ?>
				</div>
				<?php } ?>
				<div class="form_grid form_grid--wide">
					...
					foreach ( $cra_qstep['questions'] as $cra_question ) {
						...
						?>
					<label for="<?php echo esc_attr( $cra_group ); ?>"><?php echo esc_html( $cra_question['text'] ); ?></label>
						<?php echo $cra_mob_labels; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					<div class="radio_group" role="radiogroup"
						aria-label="<?php echo esc_attr( $cra_question['text'] ); ?>">
						<?php for ( $cra_i = 1; $cra_i <= CB_CRA_SCALE_MAX; $cra_i++ ) { ?>
						<input type="radio" name="<?php echo esc_attr( $cra_group ); ?>"
							data-lever="<?php echo esc_attr( $cra_question['lever_key'] ); ?>" value="<?php echo (int) $cra_i; ?>"
							class="form-check" aria-label="<?php echo (int) $cra_i; ?>">
						<?php } ?>
					</div>
						<?php
					}
		...
		?>
			<div class="form_panel">
				...
				<div class="alert alert-light">
					<?php echo wp_kses_post( $cra_contact_intro ); ?>
				</div>
				<?php } ?>
				<div class="form_grid">
				...
				<div class="form_buttons d-flex gap-2 justify-content-between">
					 ...
					 */
					?>
					<form action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post" id="craForm">
						<input type="hidden" name="action" value="cb_cra_submit">
						<input type="hidden" name="data" id="data" value="">
						<input type="hidden" name="scores" id="scores" value="">
						<input type="hidden" name="pageID" id="pageID" value="<?php echo esc_attr( get_the_ID() ); ?>">
						<input type="hidden" name="cra_token" value="<?php echo esc_attr( cb_cra_form_token() ); ?>">
...
<script>
	...
	window.addEventListener('pageshow', function(event) {
		...
		*/
		...
		<?php // phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedScript -- deliberate hand-rolled tag: cra.js carries its own ?v= cache-buster, separate from the theme version. ?>
<script src="<?php echo esc_url( get_stylesheet_directory_uri() . '/js/cra.js?v=2.3' ); ?>"></script>
		<?php
	}
);
get_footer();
