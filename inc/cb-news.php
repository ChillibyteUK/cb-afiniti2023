...
function single_sidebar() {
    ...
    // Both groups are absent entirely on posts that have never had the sidebar
    // filled in, and ACF returns false rather than an empty array, so normalise
    // before reading any keys off them.
    $opt = get_field( 'single_post_sidebar', 'options' );
    $opt = is_array( $opt ) ? $opt : array();

    $post_id  = get_the_ID();
    $override = $post_id ? get_field( 'sidebar', $post_id ) : null;
