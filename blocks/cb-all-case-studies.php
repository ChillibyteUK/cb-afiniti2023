<?php
/**
 * Block template for CB All Case Studies.
 *
...
<div class="row gx-4 gy-2 mb-4">
	...
	foreach ( $terms as $sector_term ) {
		echo '<option value=".' . esc_attr( $sector_term->slug ) . '">' . esc_html( $sector_term->name ) . '</option>';
	}
	?>
		</select>
	</div>
	...
	foreach ( $terms as $transformation_term ) {
		echo '<option value=".' . esc_attr( $transformation_term->slug ) . '">' . esc_html( $transformation_term->name ) . '</option>';
	}
	?>
		</select>
	</div>
...
<div class="<?= esc_attr( $sectclass ); ?> <?= esc_attr( $txclass ); ?> insight col-12 col-lg-4 mb-4">
	<a href="<?= esc_url( get_the_permalink() ); ?>">
		<div class="post-image-container">
			<div class="post-image mb-2" style="background-image:url('<?= esc_attr( $img ); ?>')">
