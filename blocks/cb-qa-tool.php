...
?>
<!-- qa_tool -->
<div class="container-xl pb-5 <?= esc_attr( $classes ); ?>">
	<ul class="nav tab-buttons mb-3" id="qaTabs" role="tablist">
		...
		while ( have_rows( 'section' ) ) {
			the_row();
			$st    = acf_slugify( get_sub_field( 'section_title' ) );
		...
		<li class="nav-item" role="presentation">
			<button
				class="tab-link <?= esc_attr( $active ); ?> bg--<?= esc_attr( $theme ); ?>-700"
				id="<?= esc_attr( $st ); ?>-tab" data-bs-toggle="tab"
				data-bs-target="#<?= esc_attr( $st ); ?>-tab-pane" type="button"
				role="tab" aria-controls="<?= esc_attr( $st ); ?>-tab-pane"
				aria-selected="<?= esc_attr( $selected ); ?>">
				<img src="<?= esc_url( wp_get_attachment_image_url( get_sub_field( 'icon' ), 'full' ) ); ?>"
					alt="">
				<p><?= wp_kses_post( get_sub_field( 'section_title' ) ); ?></p>
				</a>
		</li>
			<?php
			$active   = '';
	...
	<div class="tab-content qa" id="myTabContent">
		...
		while ( have_rows( 'section' ) ) {
			...
			?>
		<div class="tab-pane fade <?= esc_attr( $state ); ?>"
			id="<?= esc_attr( $st ); ?>-tab-pane" role="tabpanel"
			aria-labelledby="<?= esc_attr( $st ); ?>-tab"
			tabindex="<?= esc_attr( $ti ); ?>">

			<div
				class="bg--<?= esc_attr( $theme ); ?>-700 p-4 rounded qa__intro mb-4">
				<img src="<?= esc_url( wp_get_attachment_image_url( get_sub_field( 'icon' ), 'full' ) ); ?>"
					alt="">
				<div>
					<h2 class="qa__section">
						<?= wp_kses_post( get_sub_field( 'section_title' ) ); ?>
					</h2>
					<p><?= wp_kses_post( get_sub_field( 'section_intro' ) ); ?>
					</p>
				</div>
			</div>
			<div class="accordion" id="accordion<?= esc_attr( $ti ); ?>">
				<?php
				...
				<div class="qa__question">
					<button class="qa__button collapsed" type="button" data-bs-toggle="collapse"
						data-bs-target="#collapse<?= esc_attr( $qq ); ?>"
						id="heading<?= esc_attr( $qq ); ?>">
						<?= esc_html( get_sub_field( 'question' ) ); ?>
					</button>
					<div id="collapse<?= esc_attr( $qq ); ?>"
						class="accordion-collapse collapse"
						data-bs-parent="#accordion<?= esc_attr( $ti ); ?>">
						<div class="accordion-body" itemprop="text">
							...
							<div class="insight insight--short col-12 col-lg-4 mb-4">
								<a href="<?= esc_attr( get_the_permalink( $r ) ); ?>">
